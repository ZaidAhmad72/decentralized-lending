...
export default function DashboardPage() {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#f0f4f8] dark:bg-slate-950 flex items-center justify-center transition-colors">
    ...
    <PageTransition className="min-h-screen bg-[#f0f4f8] dark:bg-slate-950 pb-24 lg:pb-10 lg:pt-20 transition-colors">
      ...
      <div className="lg:hidden flex items-center justify-between px-5 pt-10 pb-4">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-700 flex items-center justify-center shadow-sm">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="white"><path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4z" /></svg>
          </div>
          <span className="text-slate-900 dark:text-white font-bold text-lg">Vault</span>
        </div>
        <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white text-xs font-bold px-3 py-1.5 rounded-full shadow-sm">
          {creditScore} SCORE
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-5 lg:px-10">
        ...
        <div className="mb-7">
          ...
          <h1 className="text-3xl lg:text-4xl font-black text-slate-900 dark:text-white mt-0.5">{profile.name || "—"}</h1>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6 mb-6">
          <div ref={statsRef} className="flex flex-col gap-4 lg:flex-1">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              {/* Pool Liquidity */}
              <div className="bg-white dark:bg-slate-800 rounded-3xl p-5 shadow-sm border border-slate-200/60 dark:border-slate-700/60 flex flex-col justify-between hover:shadow-md transition-shadow">
                <div className="flex items-center gap-2.5 mb-4">
                  ...
                  <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Pool Liquidity</span>
                </div>
                <div>
                  <p className="text-2xl font-black text-slate-900 dark:text-white">{formatINR(poolLiquidityINR)}</p>
                  <p className="text-sm text-emerald-600 dark:text-emerald-400 font-semibold mt-1">Available: {formatINR(availableLiquidityINR)}</p>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-slate-800 rounded-3xl p-5 shadow-sm border border-slate-200/60 dark:border-slate-700/60 flex flex-col justify-between hover:shadow-md transition-shadow">
                <div className="flex items-center gap-2.5 mb-4">
                  <div className="w-9 h-9 bg-blue-50 dark:bg-blue-900/30 rounded-xl flex items-center justify-center">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="#2563eb"><path d="M20 4H4c-1.11 0-2 .89-2 2v12c0 1.11.89 2 2 2h16c1.11 0 2-.89 2-2V6c0-1.11-.89-2-2-2zm0 14H4v-6h16v6zm0-10H4V6h16v2z" /></svg>
                  </div>
                  <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Your Deposits</span>
                </div>
                <div>
                  <p className="text-2xl font-black text-slate-900 dark:text-white">{formatINR(userDepositedINR)}</p>
                  <button onClick={() => router.push("/deposit")} className="text-xs text-blue-600 dark:text-blue-400 font-semibold mt-1 hover:underline">Deposit more →</button>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              ...
              <div className="bg-white dark:bg-slate-800 rounded-3xl p-5 shadow-sm border border-slate-200/60 dark:border-slate-700/60 flex flex-col justify-between hover:shadow-md transition-shadow">
                <div className="flex items-center gap-2.5 mb-4">
                  ...
                  <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Active Loan</span>
                </div>
                {activeLoan ? (
                  <div>
                    <p className="text-2xl font-black text-slate-900 dark:text-white">{formatINR(activeLoanINR)}</p>
                    <p className="text-sm text-emerald-600 dark:text-emerald-400 font-semibold mt-1">Due in {daysLeft} days</p>
                  </div>
                ) : (
                  <div>
                    <p className="text-base font-semibold text-slate-400 dark:text-slate-500">No active loan</p>
                    <button onClick={() => router.push("/request-loan")} className="text-xs text-blue-600 dark:text-blue-400 font-semibold mt-1 hover:underline">Borrow now →</button>
                  </div>
                )}
              </div>
              ...
              <div className="bg-white dark:bg-slate-800 rounded-3xl p-5 shadow-sm border border-slate-200/60 dark:border-slate-700/60 flex flex-col justify-between hover:shadow-md transition-shadow">
                <div className="flex items-center gap-2.5 mb-4">
                  ...
                  <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Status</span>
                </div>
                <div>
                  <p className="text-2xl font-black text-slate-900 dark:text-white capitalize">{activeLoan ? activeLoan.status : "—"}</p>
                  {activeLoan && <p className="text-sm text-emerald-600 dark:text-emerald-400 font-semibold mt-1">{activeLoan.duration_days} day term</p>}
                </div>
              </div>

              {/* Private Pool Active Loans */}
              {privateLoans.length > 0 && (
                <div className="lg:col-span-2 bg-white dark:bg-slate-800 rounded-3xl p-5 shadow-sm border border-amber-200/60 dark:border-amber-800/40">
                  <div className="flex items-center gap-2.5 mb-4">
                    ...
                    <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Private Pool Loans</span>
                    <span className="ml-auto text-xs font-bold px-2.5 py-1 rounded-full bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400">{privateLoans.length} active</span>
                  </div>
                    ...
                    {privateLoans.map((pl) => {
                      ...
                      return (
                        <button
                          ...
                          className="w-full flex items-center justify-between py-2.5 border-b border-slate-100 dark:border-slate-700 last:border-0 hover:bg-amber-50/50 dark:hover:bg-amber-900/10 rounded-xl px-2 -mx-2 transition-colors group"
                        >
                          <div className="text-left">
                            <p className="text-sm font-semibold text-slate-800 dark:text-white group-hover:text-amber-700 dark:group-hover:text-amber-400 transition-colors">{poolName}</p>
                            <p className="text-xs text-slate-500 dark:text-slate-400">Due in {daysLeft}d · {pl.amount.toFixed(4)} ETH</p>
                          </div>
                          <div className="flex items-center gap-2">
                            <div className="text-right">
                              <p className="text-sm font-black text-amber-600 dark:text-amber-400">{formatINR(amtINR)}</p>
                              <p className="text-[10px] text-slate-400">Tap to repay</p>
                            </div>
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" className="text-amber-500 opacity-60 group-hover:opacity-100 transition-opacity flex-shrink-0">
          ...
          <div ref={actionsRef} className="flex flex-col gap-3 lg:w-72 lg:flex-shrink-0">
            ...
            <button onClick={() => router.push("/deposit")}
              className="w-full bg-gradient-to-r from-emerald-500 to-green-500 hover:from-emerald-600 hover:to-green-600 text-white rounded-2xl py-4 font-bold text-base flex items-center justify-between px-5 transition-all active:scale-[0.98] shadow-md shadow-emerald-500/20">
              <div className="flex items-center gap-3">
            ...
            <button onClick={() => router.push("/request-loan")}
              className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-2xl py-4 font-bold text-base flex items-center justify-between px-5 transition-all active:scale-[0.98] shadow-md shadow-blue-500/20">
              <div className="flex items-center gap-3">
                <div className="w-7 h-7 rounded-xl bg-white/20 flex items-center justify-center">
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="white"><path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z" /></svg>
                </div>
            ...
            <button onClick={() => router.push("/repay")}
              className="w-full bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 rounded-2xl py-4 font-bold text-base flex items-center justify-between px-5 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 transition-all active:scale-[0.98]">
              <div className="flex items-center gap-3">
                <div className="w-7 h-7 rounded-xl bg-slate-100 dark:bg-slate-700 flex items-center justify-center">
            ...
            {/* Desktop stats panel */}
            <div className="hidden lg:block bg-white dark:bg-slate-800 rounded-3xl p-5 shadow-sm border border-slate-200/60 dark:border-slate-700/60 mt-1">
              <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-4">Your Stats</p>
              <div className="flex flex-col gap-2.5">
                {[
                  ...
                  { label: "Score Decay", value: scoreDecay > 0 ? `−${scoreDecay} pts` : "Active", color: scoreDecay > 0 ? "text-red-500" : "text-emerald-600" },
                  { label: "Active Loan", value: activeLoan ? formatINR(activeLoanINR) : "None" },
                  { label: "Loan Status", value: activeLoan ? activeLoan.status : "—" },
                  { label: "Days Left", value: activeLoan ? `${daysLeft}d` : "—" },
                  { label: "Gas Saved", value: `₹${gasSaved.toFixed(2)}`, color: "text-emerald-600" },
                ].map((s) => (
                  <div key={s.label} className="flex justify-between items-center py-1.5 border-b border-slate-50 dark:border-slate-700/50 last:border-0">
                    <span className="text-sm text-slate-500 dark:text-slate-400">{s.label}</span>
                    <span className={`text-sm font-semibold capitalize ${s.color || "text-slate-800 dark:text-white"}`}>{s.value}</span>
                  </div>
                ))}
              </div>
            </div>
