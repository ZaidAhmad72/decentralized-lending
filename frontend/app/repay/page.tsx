    ...
    <div className="min-h-screen bg-[#eef2f7] pb-24 md:pb-8 md:pt-20">
      ...
      <div className="flex items-center justify-between px-5 pt-10 pb-4 md:hidden">
        <div className="flex items-center gap-3">
          <button
            onClick={() => router.back()}
            className="w-9 h-9 bg-white rounded-full flex items-center justify-center shadow-sm border border-[#e5e9f0]"
          >
      ...
      <div className="max-w-2xl mx-auto px-5 md:px-8">
        ...
        <div className="mb-6">
          ...
          <p className="text-[#6b7280] text-sm">Settle your active loan and boost your reputation score.</p>
        </div>
        ...
        <div className="bg-white rounded-3xl p-5 shadow-sm border border-[#e5e9f0] mb-4">
          ...
          <div className="flex flex-col gap-3">
            <div className="flex justify-between items-center py-3 border-b border-[#f3f4f6]">
              <span className="text-sm text-[#6b7280]">Principal</span>
              <span className="font-bold text-[#111827]">
                ${loan.principal.toLocaleString("en-US", { minimumFractionDigits: 2 })}
              </span>
            </div>
            <div className="flex justify-between items-center py-3 border-b border-[#f3f4f6]">
              <span className="text-sm text-[#6b7280]">Accrued Interest</span>
              <span className="font-bold text-[#d97706]">+${loan.interest.toFixed(2)}</span>
            </div>
            <div className="flex justify-between items-center py-3 border-b border-[#f3f4f6]">
              <span className="text-sm text-[#6b7280]">Daily Rate</span>
              <span className="font-semibold text-[#374151]">{loan.dailyRate}</span>
            </div>
            <div className="flex justify-between items-center py-3 border-b border-[#f3f4f6]">
              <span className="text-sm text-[#6b7280]">Due Date</span>
              <span className="font-semibold text-[#374151]">{loan.dueDate}</span>
            </div>
        ...
        {/* Repay Button */}
        {repaid ? (
          <div className="w-full bg-[#4ade80] text-[#14532d] rounded-2xl py-5 font-bold text-lg flex items-center justify-center gap-3 mb-5">
          ...
          <button
            onClick={handleRepay}
            className="w-full bg-[#1a2fb8] text-white rounded-2xl py-5 font-bold text-lg flex items-center justify-center gap-3 hover:bg-[#1527a0] transition-all active:scale-95 mb-5"
          >
        ...
        {/* Reputation Boost Info */}
        <div className="bg-[#f0fdf4] rounded-3xl p-5 border border-[#bbf7d0] flex items-start gap-4">
